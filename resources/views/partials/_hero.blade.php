<section class="relative h-72 bg-laravel flex flex-col justify-center align-center text-center space-y-4 mb-4">
    <div class="absolute top-0 left-0 w-full h-full opacity-10 bg-no-repeat bg-center"
        style="background-image: url('images/coffeeandcupcakes-banner.png')"></div>

    <div class="z-10">
        <h1 class="text-6xl font-bold uppercase text-black">
            COFFEE SHOP<span class="text-white"> WITH CUPCAKES</span>
        </h1>
        <p class="text-2xl text-gray-800 font-bold my-4">
            Find your favorite coffee and cupcakes!
        </p>
        <div>
            <a href="/products/create"
                class="inline-block border-2 border-black text-black py-2 px-4 rounded-xl uppercase mt-2 hover:text-black hover:border-black">
                Create
            </a>
        </div>
    </div>
</section>
